...
'''
感情分析パイプラインの出力を０〜１に正規化して取り出すやつ
PPOの報酬として使うために変換している

通常のCORYの設定ではこれを生の出力値をそのまま報酬にしていたので、より安定化のために正規化するように変更

感情分析結果は
単一形式：{"label": "POSITIVE", "score": 0.9}
このように入っている
値がネガティブだったときはすべて補数をとることでプラスの値として処理をしている（IMDBデータセットにはよくある話）
'''
...
'''
移動平均を計算する関数
報酬の推移をなめらかにして学習時のノイズをへらす作戦

直近ｋ個の移動平均を計算して返すことで報酬のブレが減る
'''
