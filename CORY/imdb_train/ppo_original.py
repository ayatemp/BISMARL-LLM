...
# arti_code.add_file("/home/ayato-kaku/github/BISMARL-LLM/CORY/imdb_train/ppo.py")
# arti_code.add_dir("/home/ayato-kaku/github/BISMARL-LLM/CORY/eval", name="eval")
# arti_code.add_dir("/home/ayato-kaku/github/BISMARL-LLM/CORY/utils", name="utils")   
...
# generation_kwargs を置き換え
generation_kwargs = {
    "min_length": -1,
    "max_new_tokens": 8,    # 16 → 8（最初は短く）
    "top_k": 50,            # 0 → 50
    "top_p": 0.9,           # 1.0 → 0.9
    "temperature": 1.0,
    "do_sample": True,
...
for epoch, batch in tqdm(enumerate(ppo_trainer.dataloader)):
    ...
    pipe_outputs = sentiment_pipe(texts, **sent_kwargs)
    pos_probs = [torch.tensor(o[1]["score"], dtype=torch.float32) for o in pipe_outputs]  # POSITIVE確率

    # 3) バッチ内で簡易スケーリング（例：z-score）
    rewards_tensor = torch.stack(pos_probs)
    rewards = ((rewards_tensor - rewards_tensor.mean()) / (rewards_tensor.std() + 1e-8)).cpu().unbind()
