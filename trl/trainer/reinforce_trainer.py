...
class REINFORCETrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # ds_plugin = DeepSpeedPlugin('/home/hmp/hmp-mh/trl/ds_configs/zero_stage2.json',
        #                             zero_stage=2,
        ...
        # # note: predefined lr_scheduler
        # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, config.steps, eta_min=1.e-5)
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        if isinstance(query_tensor, List):
            ...
            # t2 = time.time()
            # print("Time for generation : {:.2f} min".format((t2 - t1) / 60))
            # self._show_gpu_memory()
        else:
    ...
    def _clear_gpu_cache(self):
        """
        Clears the cache of all available GPUs.
        """
        num_gpus = torch.cuda.device_count()
        for i in range(num_gpus):
            # torch.cuda.reset_peak_memory_stats(i)
            max_alloc = torch.cuda.max_memory_allocated(i) / 1e9
            # print("Clearing cache on GPU {}... Memory Consumption: {} GB".format(i, max_alloc))
        ...
        for i in range(num_gpus):
            torch.cuda.reset_peak_memory_stats(i)
            after_clearing_alloc = torch.cuda.max_memory_allocated(i) / 1e9
            # print("GPU {} After clearing: {} GB".format(i, after_clearing_alloc))

    def _show_gpu_memory(self):
        num_gpus = torch.cuda.device_count()
        for i in range(num_gpus):
            max_alloc = torch.cuda.max_memory_allocated(i) / 1e9
            print("GPU {} Memory Consumption: {} GB".format(i, max_alloc))
    ...
    def train_minibatch(
        ...
        """
        self.model.train()
        loss_p, train_stats = self.loss(
            old_logprobs, logits, vpreds, logprobs, mask, advantages, returns
        )
        loss = loss_p
    ...
    def loss(
        ...
        # inspired by https://arxiv.org/abs/2403.03950, replace the mse loss with HL-Gauss loss

        ratio = torch.exp(logprobs - old_logprobs)

        pg_losses = -returns * ratio
        pg_losses2 = -returns * torch.clamp(ratio, 1.0 - self.config.cliprange, 1.0 + self.config.cliprange)
        ...
        # TODO: check these stats, why are they all zeros?
        stats = dict(
            loss=dict(policy=pg_loss.detach(), total=loss.detach()),
            policy=dict(
                entropy=entropy.detach(),
                approxkl=approxkl.detach(), # 0
                policykl=policykl.detach(), # 0
                clipfrac=pg_clipfrac.detach(), # 0
                advantages=advantages.detach(),  # not logged
                advantages_mean=masked_mean(advantages, mask).detach(),  # 0
                ratio=ratio.detach(),
            ),
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        average_new_tokens = torch.mean(mask.sum(axis=-1, dtype=float))

        average_reward = (data['rewards_w_penalty'] * mask).sum(axis=-1).mean()
        ...
        stats = {
            ...
            "ppo/average_reward": average_reward,
            "temp/average_new_tokens": average_new_tokens,
        }
        ...
        # TODO: ppo/returns/mean，ppo/val/vpred与env/mean_score分别指什么
        #  env/mean_score：奖励经过clip之后的平均值
        #  ppo/returns/mean：advantage + value的平均值
        #  ppo/val/vpred: value head预测的return
        for k, v in data["train_stats"].items():
            stats[f"ppo/{k}"] = torch.mean(v)
        ...
        if self.accelerator.is_main_process:
            ...
            if self.config.log_with == "wandb":
                try:
                    ...
                    warnings.warn(f"Failed to log game logs: {e}")
            ...
            if kwargs is not None:
                # rephrase the kwargs to be more descriptive
                for k, v in kwargs.items():
                    logs[f"metrics/{k}"] = sum(v) / len(v)
