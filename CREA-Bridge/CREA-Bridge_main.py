...
- 動的バッチ調整（eff_bsz に合わせて batch / mini-batch を整合）
- IRM: 生logitsヒスト、Platt/μσキャリブ、穏当なフォールバック（ztemp 温度上限=2）
...
- Bridger → Pioneer → Observer の3段生成 + IRMスコア + PPO更新（バッチ対応）
- フォーマットエラー対策：str.format 廃止 → [[...]] 置換に統一
- BRIDGER: "keys" 検出 + JSON未完救済 + 疑似リスト救済 + 最低5キー保障
- policy は GPU0, IRM は GPU1 固定
- 生成は policy.generate を直叩き（CUDA:0 / use_cache=True）
- PPOは KL=0（reference-free相当）。有効サンプル数に合わせて trainer.config.batch_size を動的に上書き
- IRM 報酬: raw/sigmoid/calib（reward_calibration.json があれば calib 優先）
- W&B: reward系統計＋samples/outputsテーブル
...
- Bridger → Pioneer → Observer の3段生成 + IRMスコア + PPO更新（バッチ対応）
- 出力は "論文タイトル + アブスト" 形式に統一（IRM 分布合わせ）
- IRM: 生logits出力/ヒストグラム、z正規化+温度 or Platt 校正に対応
- 報酬スケールの固定キャリブ（--irm-calib-path）に対応（推奨）
- W&B: reward系統計、logitsヒスト、累積 samples/outputs テーブル
- 生成は policy.generate を直叩き（CUDA:0）; KLは小さめに計測/制御可能
- BRIDGER: 鍵が弱いサンプルのスキップ + スキップ率ログ
- str.format 廃止 → [[...]] 置換で安全化
- TorchInductor/Triton を環境変数で無効化（triton未導入環境の安定化）
"""
...
GEN_BRIDGER = GenCfg(max_new_tokens=48,  temperature=0.0, top_p=1.0, top_k=0)
GEN_PIONEER  = GenCfg(max_new_tokens=96,  temperature=0.2, top_p=0.9, top_k=0)
GEN_OBSERVER = GenCfg(max_new_tokens=96,  temperature=0.2, top_p=0.9, top_k=0)
...
class IRMScorer:
    ...
    def score_text(self, title: str, abstract: str,
        ...
        reward_mode:
          ...
          - "logit": 生logitを直接（>0 大、<0 小）→ PPOに生値は推奨しない
        ...
        # 多クラス対応: 最大logitに簡易化（IRMが多クラスでも最も確信の強いクラスを採用）
        ...
        if reward_mode == "logit":
            ...
            # 純確率：二値→sigmoid(logit)、多クラス→softmax(max) 相当
            # ここでは二値想定の近似として sigmoid(logit) を使う
...
def run_pipeline_three_agents(bundle: ModelBundle, task: str) -> Dict[str, Any]:
    ...
    bridger_out = run_generate_cuda(bundle.model, bundle.tokenizer, bridger_prompt, GEN_BRIDGER, stopper=JSONKeysStopper(bundle.tokenizer))
    bj = extract_first_json(bridger_out)
    bridger_json = _enforce_keys_only_json(bj) if bj is not None else {"keys": []}
    if not bridger_json.get("keys"):
        salv = _salvage_keys_from_text(bridger_out)
        if salv: bridger_json["keys"] = salv
    if not bridger_json.get("keys"):
    ...
    # PIONEER
    pioneer_prompt = _fill(PIONEER_PROMPT, {"TASK": task, "KEYS": json.dumps(bridger_json.get("keys", []), ensure_ascii=False)})
    ...
    pioneer_json = {"ideas": ideas, "rationale": rationale}

    # OBSERVER
    observer_prompt = _fill(OBSERVER_PROMPT, {"TASK": task, "PIONEER_JSON": json.dumps(pioneer_json, ensure_ascii=False)})
    ...
    observer_json = {"ideas": o_ideas, "rationale": o_rat}

    return {
        "bridger_raw": str(bridger_out),
        "bridger": {"keys": [str(x) for x in (bridger_json.get("keys") or [])]},
...
def main():
    ...
    ap.add_argument("--reward-temp", type=float, default=4.0)  # ztempの温度
    ...
    # PPO config (light KL to ensure movement)
    ppo_cfg = PPOConfig(
        ...
        seed=42,
        init_kl_coef=0.02,   # measure & softly constrain
        target_kl=0.05,
    ...
    for upd in range(total_updates):
        ...
        for b in range(args.batch_size):
            task = prompts[next_idx % len(prompts)]; next_idx += 1
            try:
                out = run_pipeline_three_agents(bundle, task)
            except Exception as e_pipeline:
                print(f"[DEBUG] pipeline failed at upd {upd+1} (b={b}): {repr(e_pipeline)}")
                out = {"observer": {"ideas": []}, "observer_raw": "", "bridger": {"keys": []}}

            # Skip if keys too few
            keys_now = out.get("bridger",{}).get("keys",[])
            if not isinstance(keys_now, list): keys_now=[]
            if len(keys_now) < 3:
                skipped_cnt_total += 1
                continue

            # choose idea
            obs_ideas = out.get("observer",{}).get("ideas",[]) if isinstance(out, dict) else []
            if obs_ideas:
            ...
            try:
                ...
                # clip lengths for PPO stability
                ...
                # reward (IRM) + logits logging
                rew_val = irm.score_text(title, abstract, reward_mode=args.reward_mode, reward_temp=args.reward_temp)
                rew_t = torch.tensor(float(rew_val), dtype=torch.float32, device="cuda:0")
        ...
        if eff_bsz == 0:
            ...
            if pbar:
                pbar.update(1); pbar.set_postfix({"epoch": f"{args.ppo_epochs}/{args.ppo_epochs}", "reward":"NaN","bsz":0})
            # even if skipped, log logits=empty? skip
            continue

        # PPO step
        try:
            print(f"[DBG] effective_batch_size={eff_bsz}, ppo(batch={args.batch_size}, mini={args.mini_batch_size})")
        ...
        if _HAS_WANDB:
            ...
            # samples/outputs: accumulate into one table; log at human-friendly cadence
            if (upd + 1) % max(1, args.log_every_n) == 0:
                try:
                    lod = last_out_dict if isinstance(last_out_dict, dict) else {}
                    if samples_table is not None:
                        samples_table.add_data(
                            upd + 1,
                            (last_task_str or "")[:2000],
                            safe_get(lod, "bridger_raw", "")[:2000],
                            safe_get(lod, "pioneer_raw", "")[:2000],
                            safe_get(lod, "observer_raw", "")[:2000],
                        )
                        wandb.log({"samples/outputs": samples_table}, step=upd + 1)
                except Exception as e:
